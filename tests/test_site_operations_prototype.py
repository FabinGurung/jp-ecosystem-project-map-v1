...
class SiteOperationsPrototypeTests(unittest.TestCase):
    def test_ceo_tabs_and_assets_are_wired(self) -> None:
        ...
        self.assertIn(
            'Resources <span class="tab-count" id="materials-tab-count">',
            html,
        )

        self.assertIn(
            "assets/css/site-operations-prototype.css",
            html,
        )

        self.assertIn(
            "assets/js/site-operations-prototype.js",
            html,
        )

    def test_demo_is_explicit_and_scoped(self) -> None:
        source = (
            ROOT / "assets/js/site-operations-prototype.js"
        ...
        self.assertIn(
            "These schedule, progress, equipment and stock values are not read from Drive",
            source,
        )
    ...
    def test_private_inventory_gate_remains_closed(self) -> None:
        config = json.loads(
            (ROOT / "map-config.json").read_text(encoding="utf-8")
        )

        self.assertFalse(
            config["features"]["show_inventory_quantities"]
        )
    ...
    def test_no_live_schedule_or_inventory_rows_are_committed(self) -> None:
        for filename in [
            ...
            self.assertEqual(
                rows,
                [],
                f"{filename} should remain header-only in the public prototype",
            )

    def test_workflow_validates_new_branch_and_module(self) -> None:
        workflow = (
            ROOT / ".github/workflows/validate-projects.yml"
        ).read_text(encoding="utf-8")

        self.assertIn(
            "v1.2-site-operations-dashboard",
            workflow,
        )

        self.assertIn(
            "node --check assets/js/site-operations-prototype.js",
            workflow,
        )
